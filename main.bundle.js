...
/***/ (function(module, exports) {
	...
	function buildCard(options) {
	  ...
	  const arrowBox = $(`<div class="quality-arrows"><p class="card-quality" id=${options.qualityIndex || 2} tabindex="0">quality: ${cardQualities[options.qualityIndex || 2]}</p><div>`)
	...
	function addCompleteCheckbox(card, completed) {
	  let checkbox
	  if (completed) {
	    checkbox = $(`<label class="checkbox"><input class="checkbox-input" role="checkbox" aria-checked="true" checked="checked" type="checkbox" value="complete-task">complete task</label>`)
	  } else {
	    checkbox = $(`<label class="checkbox"><input class="checkbox-input" role="checkbox" aria-checked="false" type="checkbox" value="complete-task">complete task</label>`)
	  }
	...
	function createButton(classes, value) {
	  return $('<button class="'+classes+'" aria-label="'+value+'"></button>');
	}

	function addDeleteButton(card) {
	  const deleteBtn = createButton('close-card', 'delete');
	...
	function addUpVoteButton(arrowBox) {
	  const btn = createButton('up-arrow', 'up-vote');
	...
	function addDownVoteButton(arrowBox) {
	  const btn = createButton('down-arrow', 'down-vote');
	  arrowBox.prepend(btn);
	  btn.on('click',function(){
	    downVote(arrowBox);
	  });
